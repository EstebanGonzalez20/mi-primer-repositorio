...
public class Controlador implements ActionListener {
    ...
    public Controlador() {
        ...
        vista.getPuertosButton().addActionListener(this);
        vista.getRutasButton().addActionListener(this);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == vista.getIniciarButton()) {
            ...
            if (fin.isEmpty() || inicio.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Error, debe haber una IP de principio y de fin", "Error",
                        JOptionPane.ERROR_MESSAGE);
            }

            else if (IP.esMayorQue(inicio, fin)) {
                JOptionPane.showMessageDialog(null, "Error, ingrese la IP más grande al como fin del rango", "Error",
                        JOptionPane.ERROR_MESSAGE);
            }
                ...
                SwingWorker<Void, Object[]> worker = new SwingWorker<Void, Object[]>() {
                    ...
                    protected Void doInBackground() throws Exception {
                        ...
                        while (true) {
                            ...
                            publish(new Object[] { ipActual.getIpCompleta(), nombre, coneccion, tiempo }); // Publica
                                                                                                           // resultados
                    ...
                    @Override
                    protected void done() {
                        JOptionPane.showMessageDialog(null, "Proceso completado", "Info",
            ...
            if (estado != null) {
                ...
                sorter.setRowFilter(javax.swing.RowFilter.regexFilter(estado, 2)); // Filtra por la columna "Estado"
                                                                                   // (índice 2)
            }
        }

        else if (e.getSource() == vista.getPuertosButton()) {
            List<String> resultado = CMD.ejecutarComando("netstat -an");
            VistaPuertos vp = new VistaPuertos(vista, resultado.subList(4, resultado.size()));
        }

        else if (e.getSource() == vista.getRutasButton()) {
            List<String> resultado = CMD.ejecutarComando("netstat -rn");
            ...
            for (String linea : resultado) {
                if (linea.trim().matches("^=+$")) {
                    // Si hay contenido en el bloque, lo guardamos
                    if (!bloqueActual.isEmpty()) {
                        sublistas.add(new ArrayList<>(bloqueActual));
                        bloqueActual.clear();
                    }
                } else {
            ...
            if (!bloqueActual.isEmpty()) {
                sublistas.add(bloqueActual);
            }
             VistaRutas vr = new VistaRutas(vista, sublistas);
            // resultado.size()));
        }
    }
